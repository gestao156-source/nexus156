-- Sistema de Notificações Nexus156 - VERSÃO FINAL CORRIGIDA
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  type VARCHAR(50) NOT NULL CHECK (type IN (
    'new_item', 'status_change', 'assignment', 'system_alert'
  )),
...
-- Trigger para atualizar updated_at - CORRIGIDO
...
-- Função para criar notificação (RPC) - VERSÃO CORRIGIDA
DROP FUNCTION IF EXISTS create_notification(p_user_id UUID, p_type VARCHAR(50), p_title VARCHAR(255), p_message TEXT, p_metadata JSONB, p_priority VARCHAR(10), p_action_url TEXT, p_action_text VARCHAR(100);
...
BEGIN
  ...
  IF user_prefs.quiet_hours_enabled AND should_create THEN
    IF v_current_time >= user_prefs.quiet_hours_start AND 
       v_current_time <= user_prefs.quiet_hours_end THEN
...
DROP FUNCTION IF EXISTS mark_notification_read(p_notification_id UUID, p_user_id UUID);
...
DROP FUNCTION IF EXISTS mark_all_notifications_read(p_user_id UUID);
...
DROP FUNCTION IF EXISTS get_user_notifications(p_user_id UUID, p_limit INTEGER, p_offset INTEGER, p_status VARCHAR(20), p_type VARCHAR(50));
...
DROP FUNCTION IF EXISTS count_unread_notifications(p_user_id UUID);
