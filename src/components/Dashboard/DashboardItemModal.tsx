...
export default function DashboardItemModal({ 
  ...
  const getModalBehavior = () => {
    ...
    if (items.length <= 5) return 'compact';
    return 'search';
  };

  const behavior = getModalBehavior();

  // Se não tiver itens ou for um item único, não abre modal
  if (behavior === 'empty' || behavior === 'single') {
    return null;
  }
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">
              {items.length} {status === 'aguardando' ? 'Aguardando Análise' : 
