-- Migration: Criar tabela de notificações (VERSÃO CORRIGIDA)
...
-- Usuários só podem inserir notificações para si mesmos (ou admin pode para todos)
...
-- Função para criar notificação (RPC) - CORRIGIDA
...
BEGIN
  ...
  -- Verificar quiet hours - CORRIGIDO
  IF user_prefs.quiet_hours_enabled THEN
    IF current_time >= user_prefs.quiet_hours_start AND 
      ...
      should_create := should_create AND (p_priority = 'urgent');
    END IF;
  END IF;
