-- Migration: Criar triggers automáticos para notificações (VERSÃO DEFINITIVA 100% CORRIGIDA)
...
BEGIN
  ...
  -- Obter data do prazo (tentar diferentes nomes de coluna)
  BEGIN
    prazo_date := NEW.prazo;
  EXCEPTION WHEN undefined_column THEN
    BEGIN
      prazo_date := NEW.data_prazo;
    EXCEPTION WHEN undefined_column THEN
      BEGIN
        prazo_date := NEW.data_limite;
      EXCEPTION WHEN undefined_column THEN
        RETURN NEW; -- Se não encontrar nenhuma coluna de prazo, não fazer nada
      END;
    END;
  END;
  
  -- Calcular dias até o prazo
  SELECT (prazo_date - CURRENT_DATE) INTO days_until_deadline;
...
BEGIN
  ...
  SELECT (CURRENT_DATE - prazo_date) INTO days_overdue;
...
CREATE TRIGGER trigger_solicitacao_deadline_warning
  AFTER UPDATE ON solicitacoes
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status OR OLD.prazo IS DISTINCT FROM NEW.prazo OR NEW.prazo IS NOT NULL)
...
BEGIN
  ...
  FOR item IN 
    SELECT id, responsavel, assunto, protocolo, prazo, 'solicitacao' as item_type
    FROM solicitacoes
    WHERE status != 'finalizado'
      AND prazo IS NOT NULL
      AND prazo BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '3 days')
  ...
  FOR item IN 
    SELECT id, responsavel, assunto, protocolo, prazo, 'demanda' as item_type
    FROM demandas
    WHERE status != 'finalizado'
      AND prazo IS NOT NULL
      AND prazo BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '3 days')
