-- Migration: Criar triggers automáticos para notificações
...
BEGIN
  ...
  IF NEW.responsavel IS NOT NULL AND is_valid_uuid(NEW.responsavel) THEN
    SELECT create_notification(
      NEW.responsavel,
      'assignment',
...
BEGIN
  ...
  IF NEW.responsavel IS NOT NULL AND is_valid_uuid(NEW.responsavel) 
...
-- Trigger para avisos de prazo (baseado em data_contato)
CREATE OR REPLACE FUNCTION trigger_deadline_warning_notification()
RETURNS TRIGGER AS $$
DECLARE
  days_until_deadline INTEGER;
BEGIN
  -- Calcular dias até o prazo usando data_contato
  SELECT (NEW.data_contato - CURRENT_DATE) INTO days_until_deadline;
...
CREATE TRIGGER trigger_solicitacao_deadline_warning
  AFTER UPDATE ON solicitacoes
  FOR EACH ROW
  WHEN (OLD.data_contato IS DISTINCT FROM NEW.data_contato OR NEW.data_contato IS NOT NULL)
...
CREATE TRIGGER trigger_solicitacao_deadline_passed
  AFTER UPDATE ON solicitacoes
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status OR NEW.data_contato IS NOT NULL)
...
BEGIN
  ...
  FOR item IN 
    SELECT id, responsavel, assunto, protocolo, data_contato, 'solicitacao' as item_type
    FROM solicitacoes
    WHERE status != 'finalizado'
      AND data_contato IS NOT NULL
      AND data_contato BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '3 days')
  ...
  FOR item IN 
    SELECT id, responsavel, assunto, protocolo, data_contato, 'demanda' as item_type
    FROM demandas
    WHERE status != 'finalizado'
      AND data_contato IS NOT NULL
      AND data_contato BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '3 days')
